<template>
  <div>
    <h1>Xin chào, {{ this.editAccount.name }}</h1>
    <div v-if="!accountLoading" class="row">
      <div class="col-4 text-left">
        <div>
          <label>
            Tài khoản:
            <input class="form-control" v-model="editAccount.admin_id" disabled />
          </label>
          <label>
            Mật khẩu:
            <input class="form-control" :value="'thisispassword'" type="password" disabled />
          </label>
          <button
            @click="dialog = true"
    ...
    <v-dialog :value="true" width="40rem" persistent>
      <v-card>
...
export default {
  ...
  methods: {
    editInfor() {
      let vm = this;
      let adminObj = {
        name: vm.editAccount.name,
        phone: vm.editAccount.phone,
        address: vm.editAccount.address
      };
      this.$swal({
        icon: "question",
        title: "Thông Báo",
        ...
        if (result.value) {
          vm.$store.dispatch("adminEditInfor", adminObj).then(response => {
            let icon = "";
    ...
    changePassword() {
      let vm = this;
      let admin = {
        admin_id: this.editAccount.admin_id,
        password: this.currentPassword,
        newPassword: this.newPassword
      };
      //console.log('dasd',admin)
      this.$store.dispatch("adminChangePassword", admin).then(response => {
        //console.log("dsad",admin)
